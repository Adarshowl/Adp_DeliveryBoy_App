...
const OnTheWayDelivery = ({ navigation }) => {
  ...
  const getAllShop = () => {
    setLoading(true);
    try {
      console.log("response axios >>> ", JSON.stringify(API_END_POINTS.ON_The_Way_OrderList));
  ...
  const updateStatus = (item) => {
    ...
    try {
      if (selectedOption !== 'UnPaid' && selectedOption !== 'Paid') {
        ShowToastMessage("Please select 'Paid' or 'UnPaid' before updating the order status.");
        setLoading(false);
        return null;
      }
      const body = {
  ...
  const onLoginClick = (item) => {
    ...
    try {
      if (!buttonClicked || item?.orderId?.payment_status === 'Paid') {
        const selectedOption = item?.orderId?.payment_status === 'Paid' ? 'Paid' : '';
        ...
        if (selectedOption === 'Paid') {
          ...
          ShowToastMessage("Please select Payment Status before updating the order");
          setLoading(false);
        }
      } else {
        ...
        {dashboard.length !== 0 && (
          ...
          <View style={{
            ...
            <View style={{
              ...
              <Image
                source={{ uri: 'https://cdn-icons-png.flaticon.com/128/8901/8901370.png' }}
